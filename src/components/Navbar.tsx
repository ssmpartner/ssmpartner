          ...
          <div className="hidden lg:flex items-center gap-10">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className="font-body text-sm tracking-wide text-foreground/70 hover:text-foreground transition-colors relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-px after:bg-accent after:scale-x-0 after:origin-left after:transition-transform after:duration-300 hover:after:scale-x-100"
              >
                {link.label}
              </Link>
            ))}

            <div className="flex items-center gap-1 text-xs font-body text-muted-foreground ml-4">
              {languages.map((l, i) => (
                <span key={l} className="flex items-center">
                  <button
                    onClick={() => setLang(l)}
                    className={`px-1 py-0.5 uppercase transition-colors ${
                      lang === l ? "text-accent font-medium" : "hover:text-foreground"
                    }`}
                  >
                    {l}
                  </button>
            ...
            <Link
              to="/kontakt"
              className="font-body text-sm border border-foreground px-5 py-2 text-foreground hover:bg-foreground hover:text-background transition-all duration-300"
            >
              {t("nav.cta")}
            </Link>
          </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            <div className="flex items-center gap-2 text-sm font-body text-muted-foreground mt-4">
              {languages.map((l, i) => (
                <span key={l} className="flex items-center">
                  <button
                    onClick={() => setLang(l)}
                    className={`px-1 uppercase ${lang === l ? "text-accent font-medium" : ""}`}
                  >
                    {l}
                  </button>
            ...
            <Link
              to="/kontakt"
              className="font-body text-sm border border-foreground px-6 py-3 text-foreground hover:bg-foreground hover:text-background transition-all"
            >
              {t("nav.cta")}
            </Link>
          </motion.div>
